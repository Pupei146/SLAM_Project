...
void Viewer::UpdateMap() {
    ...
    active_landmarks_ = map_->getAllActiveLandmarks();
    map_updated_ = true;
}
...
cv::Mat Viewer::PlotFrameImage() {
    cv::Mat img_out;
    cv::cvtColor(current_frame_->left_image, img_out, cv::COLOR_BGRA2GRAY);
    for (size_t i = 0; i < current_frame_->features_left.size(); ++i) {
        if (current_frame_->features_left[i]->map_point.lock()) {
            auto feat = current_frame_->features_left[i];
            cv::circle(img_out, feat->position.pt, 2, cv::Scalar(0, 250, 0),
                       2);
        }
    }
    return img_out;
